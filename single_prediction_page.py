...
def show_single_page():
    ...
    #List variables for pridictions
    species = ['Harbour porpoise',"Dall's porpoise", 'Pacific white-sided dolphin', 'Killer whale',
                "Grey whale", 'Humpback whale', 'Minke whale', 'Fin whale',  'Sperm whale', "Risso's dolphin",
                'Northern right whale dolphin', 'False killer whale', "Baird's beaked whale", "Cuvier's beaked whale",
                'Sei whale', 'Blue whale', 'North Pacific right whale', 'Other rare species', 'Unidentified dolphin or porpoise',
                'Unidentified whale', 'Leatherback sea turtle', 'Green sea turtle', 'Oliver ridley sea turtle',
                'Loggerhead sea turtle', 'Unidentified sea turtle']
    sea = ['Sea is smooth', 'Light air, small ripples', 'Wind felt on face;wave crests have glassy appearance',
                'Crests begin to break;whitecaps start to form at 10 knots', 'Frequent whitecaps;sea to 3 ft (1 m)',
                'Moderate waves form 4-5 ft (1.5-2.5 m);some spray carried', 'Large waves 5-7 ft (2-3 m);more spray',
                'Swell forms 8-10 ft (3-4 m);foam blown in streaks', 
                'Moderately high waves to 16 ft (5 m);crests break into spindrift',
                'High waves to 20 ft (7 m);dense foam, visibility reduced', 'Exceptionally high waves to 30 ft (9 m)',
                'Waves to 35 ft (11 m);very limited visibility', 'Waves to 50 ft (15m);air filled with foam and spray']
    wind = ['0 kts (Calm)', '1-3 kts (Light Air)', '4-6 kts (Light Breeze)', '7-10 kts (Gentle Breeze)',
                '11-16 kts (Moderate Breeze)', '17-21 kts (Fresh Breeze)', '22-27 kts (Strong Breeze)',
                '28-33 kts (Near Gale)', '34-40 kts (Gale)', '41-47 kts (Strong Gale)', '48-55 kts  (Storm)',
                '56-63 kts (Violent Storm)', '64+ kts (Hurricane)']
    measure = ['Exact', 'Approximate','Range']
    behaviour = ['Fluking', 'Feeding', 'Porpoising', 'Spy-hopping', 'Slow Moving', 'Diving', 
                'Depredation', 'Travelling quickly', 'Bow-riding', 'Breaching', 'Fast moving']
    distance = ['< 25m (27yd/82ft)', '25-100m (27yd/82ft-109yd/328ft)','100-300m (109yd/328ft-328yd/984ft)',
                '300-1000m (328yd/984ft-1093yd/3280ft)', '> 1km (>1093yd/3280ft)']
    direction = ['North', 'East', 'West', 'South', 'North-West', 'North-East','South-West', 'South-East', 'Various', 'Unknown']
    platform = ['Kayak', 'Sailboat', 'Motor Vessel <25 ft', 'Motor Vessel 25-60 ft', 'Motor Vessel >60 ft',
                'Ferry', 'Airplane/Helicopter', 'Land (close to water)', 'Land (high above water)', 'Other']
    experience = ['Novice recreational observer', 'Experienced recreational observer',
                'Other marine professional', 'Researcher', 'Lightstation keeper']

    #Create input for features
    SpeciesName = st.selectbox('Species Name', species)
    LatitudeDD = st.number_input('Sighting Latitude')
    LongitudeDD = st.number_input('Sighting Longitude')
    Date = st.date_input('Date of sighting')
    Time = st.time_input('Time of sighting')
    SeaState = st.selectbox('What was the sea state?', sea)
    WindSpeed = st.selectbox('What was the wind speed?', wind)
    NumberOfAnimalsMin = st.number_input('Number of Animals')
    AnimalCountMeasure = st.selectbox('How sure are you in your estimation?', measure)
    Behaviour = st.selectbox('What behaviour was the animal exhibiting?', behaviour)
    SightingDistance = st.selectbox('How far was the animal?', distance)
    DirectionOfTravel = st.selectbox('What direction was the animal traveling?', direction)
    SightingPlatform = st.selectbox('Where were you?', platform)
    Experience = st.selectbox('What is your experience?', experience)
    ...
    features = {
        ...
        'SeaState': SeaState,
        'WindSpeed': WindSpeed,
        'AnimalCountMeasure': AnimalCountMeasure,
        'Number of Animals Min': NumberOfAnimalsMin,
        'Behaviour':Behaviour,
        'SightingDistance':SightingDistance,
